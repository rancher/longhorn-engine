...
func (s *SyncAgentServer) BackupCreate(ctx context.Context, req *ptypes.BackupCreateRequest) (*ptypes.BackupCreateResponse, error) {
	...
	backupID, replicaObj, err := backup.DoBackupCreate(req.BackupName, req.VolumeName, req.SnapshotFileName, req.BackupTarget, req.BackingImageName, req.BackingImageUrl, req.Labels)
	if err != nil {
...
func (s *SyncAgentServer) BackupRestore(ctx context.Context, req *ptypes.BackupRestoreRequest) (e *empty.Empty, err error) {
	...
	requestedBackupName, _, _, err := backupstore.DecodeMetadataURL(util.UnescapeURL(req.Backup))
	if err != nil {
		return nil, err
	}
